var profit = {
	...
	createCertificateUploader : function(csrfToken){
		...
		$('#certificateFile').on('fileuploaderror', function(event, data, msg) {
		    profit.showErrorDialog(messages.errorUploadCertificate);
		});
	},
	...
	moreProfiles : function(searchQuery) {
		...
		$.ajax({
			...
			error : function(data) {
				profit.showErrorDialog(messages.errorAjax);
			}
		});
	},
	...
	ui : {
		...
		getTemplate : function() {
			if (profit.ui.template == null) {
				var source = $("#ui-block-template").html();
				profit.ui.template = Handlebars.compile(source);
			}
			return profit.ui.template;
		},

		addBlock : function() {
			var template = profit.ui.getTemplate();
		...
		save : function() {
			...
			if (selectedHobbyButtons.length > maxHobbies) {
				var closeFunction = function() {
					...
					if (profit.hobbies.errorTimeout != null) {
						clearTimeout(profit.hobbies.errorTimeout);
						profit.hobbies.errorTimeout = null;
					}
				};
				$('#errorAlert button').unbind('click');
				$('#errorAlert button').click(closeFunction);
				$('#errorAlert').fadeIn('slow');
				profit.hobbies.errorTimeout = setTimeout(closeFunction, 5000);
				return;
			}
