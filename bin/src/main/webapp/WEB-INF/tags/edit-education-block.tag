...
<%@ attribute name="education" required="false" type="com.ab.profit.entity.Education"%>
...
<div id="ui-item-${index }" class="panel panel-default">
	...
	<div class="panel-body ui-item">
		...
		<div class="row">
			<c:if test="${education != null }"><profit:form-has-error path="items[${index }].summary"/></c:if>
			<div class="col-xs-12 col-md-12 ${hasError ? 'has-error has-feedback' : ''}">
				<label class="control-label" for="items${index}summary">Specialization</label> 
				...
				<profit:form-error path="items[${index }].summary" />
				</c:if>
			</div>
			<div class="col-xs-6 form-group">
				<label for="items${index }beginDate">Begin date</label> 
			...
			<c:if test="${education != null }"><profit:form-has-error path="items[${index }].finishYear"/></c:if>
			<div class="col-xs-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				<label for="items${index }finishDate">Finish date</label> 
				...
				<c:if test="${education != null }">
				<profit:form-error path="items[${index }].finishYear" displayIcon="false"/>
				</c:if>
			</div>
			<c:if test="${education != null }"><profit:form-has-error path="items[${index }].university"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				<label class="control-label" for="items${index }university">University</label> 
				...
				<c:if test="${education != null }">
				<profit:form-error path="items[${index }].university" />
				</c:if>
			</div>
			<c:if test="${education != null }"><profit:form-has-error path="items[${index }].faculty"/></c:if>
			<div class="col-xs-12 col-md-6 form-group ${hasError ? 'has-error has-feedback' : ''}">
				<label class="control-label" for="items${index }faculty">Faculty</label> 
				<input class="form-control" name="items[${index }].faculty" id="items${index }faculty" 
										value="${education.faculty }" required="required">
				<c:if test="${education != null }">
				<profit:form-error path="items[${index }].faculty" />
				</c:if>
			</div>
		</div>
	</div>
