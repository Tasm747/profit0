...
<%@ attribute name="course" required="false" type="com.ab.profit.entity.Course"%>
...
<div id="ui-item-${index }" class="panel panel-default">
	...
	<div class="panel-body ui-item">
		...
		<div class="row">
			<c:if test="${course != null }"><profit:form-has-error path="items[${index }].name"/></c:if>
			<div class="col-xs-6 col-md-4 form-group ${hasError ? 'has-error has-feedback' : ''}">
				...
				<profit:form-error path="items[${index }].name" />
				</c:if>
			</div>
			<c:if test="${course != null }"><profit:form-has-error path="items[${index }].school"/></c:if>
			<div class="col-xs-6 col-md-4 form-group ${hasError ? 'has-error has-feedback' : ''}">
				<label class="control-label" for="items${index }school">School name / resources*</label> 
				...
				<c:if test="${course != null }">
				<profit:form-error path="items[${index }].school" />
				</c:if>
			</div>
